...
const SearchBar = () => {
  ...
  const handleSearch = () => {
    ...
    router.push({
      // Sesuaikan pathname menjadi huruf kecil agar cocok dengan nama file baru
      pathname: "/list-kosan",
      query: query,
    });
  };
  ...
  const handleMinBudgetChange = (e) => {
    ...
    setMinBudget(newMin);
    // Jika min baru melebihi max, set max sama dengan min baru
    if (newMin > maxBudget) {
      setMaxBudget(newMin);
    }
  };
  ...
  const handleMaxBudgetChange = (e) => {
    ...
    setMaxBudget(newMax);
    // Jika max baru lebih kecil dari min, set min sama dengan max baru
    if (newMax < minBudget) {
      setMinBudget(newMax);
    }
  };
